...
def load(f, _dict=dict, decoder=None):
    ...
    Raises:
        ...
        FileNotFoundError -- When an array with no valid (existing) file paths is passed
    """

    if isinstance(f, (str, bytes, pathlib.PurePath)):
        with io.open(os.fspath(f), encoding='utf-8') as ffile:
            return loads(ffile.read(), _dict, decoder)
    elif isinstance(f, list):
        from warnings import warn
        if not [path for path in f if os.path.exists(path)]:
